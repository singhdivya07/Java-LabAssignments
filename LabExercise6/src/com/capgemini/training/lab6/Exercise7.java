...
public class Exercise7 {
	public static class ArrayListDemo {
		...
		public static void main(String[] args) {
			...
			/*
			 * for(Integer m: checkList) { System.out.println(m); } }
			 */
		/*
		 * private static Integer[] getSortedAgain() { Integer [] list= new Integer[4];
		 * list[0]=205; list[1]=115; list[2]=501; list[3]=101; String [] reverseNumbers
		 * = new String[list.length]; for(int i=0;i<list.length;i++) {
		 * reverseNumbers[i]= list[i].toString(); StringBuffer sb= new
		 * StringBuffer(reverseNumbers[i]); reverseNumbers[i]=sb.reverse().toString();
		 * list[i]= Integer.parseInt(reverseNumbers[i]); }
		 * 
		 * Arrays.sort(list); return list; }
		 */
